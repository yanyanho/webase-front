...
contract AssetNetwork is Utils, BAC001Holder{
    // Instance of the asset used by the channels
    BAC001 public asset;
    ...
    uint256 public asset_network_deposit_limit;
...
//    string public constant signature_prefix = '\x19Ethereum Signed Message:\n';
    ...
    event WithdrawEvent( address indexed participant,bytes32 messageHash);
    ...
    /// @param _asset_address The address of the BAC001 asset contract.
    /// @param _secret_registry The address of SecretRegistry contract that witnesses the onchain secret reveals.
    /// @param _chain_id  Chain ID of the blockchain where this instance is being deployed.
    ...
    /// that can be chosen at the channel opening.
    /// @param _deprecation_executor The  address that can disable new deposits and channel creation.
    /// @param _channel_participant_deposit_limit The maximum amount of assets that can be deposited by each
    /// participant of each channel. MAX_SAFE_UINT256 means no limits.
    /// @param _asset_network_deposit_limit The maximum amount of assets that this contract can hold.
    ...
    constructor(
        ...
        public
    {
        require(_asset_address != address(0x0));
        ...
        require(contractExists(_asset_address));
        require(contractExists(_secret_registry));
        require(_channel_participant_deposit_limit > 0);
        require(_asset_network_deposit_limit > 0);
        require(_asset_network_deposit_limit >= _channel_participant_deposit_limit);

        asset = BAC001(_asset_address);
        ...
        // Make sure the contract is indeed a asset contract
        require(asset.totalAmount() > 0);

        deprecation_executor = _deprecation_executor;
        channel_participant_deposit_limit = _channel_participant_deposit_limit;
        asset_network_deposit_limit = _asset_network_deposit_limit;
    }
        ...
        returns (uint256)
    {
        ...
        require(asset.balance(address(this)) < asset_network_deposit_limit);
    ...
    /// @param total_deposit The total amount of assets that the participant
        ...
        isOpen(channel_identifier)
    {
        ...
        // Calculate the actual amount of assets that will be transferred
        added_deposit = total_deposit - participant_state.deposit;

        // The actual amount of assets that will be transferred must be > 0
        ...
        // the participant_state.deposit = total_deposit, while we transfer `added_deposit` assets.
        assert(participant_state.deposit + added_deposit == total_deposit);

        require(asset.balance(address(this)) + added_deposit <= asset_network_deposit_limit);
        ...
        // Do the transfer
       asset.sendFrom(msg.sender, address(this), added_deposit,"");
    }

    /// @notice Allows `participant` to withdraw assets from the channel that he
    ...
    /// amount.
    /// @param total_withdraw Total amount of assets that are marked as
        ...
        isOpen(channel_identifier)
    {
        ...
        require(channel_identifier == getChannelIdentifier(participant, partner),"channel_identifier checkout failed");
        ...
        // The actual amount of assets that will be transferred must be > 0 to disable the reuse of
        // withdraw messages completely.
        require(current_withdraw > 0);

        // This should never fail at this point. Added check for security, because we directly set
        // the participant_state.withdrawn_amount = total_withdraw,
        // while we transfer `current_withdraw` assets.
        ...
        // Do the state change and assets transfer
        participant_state.withdrawn_amount = total_withdraw;
        asset.send(participant, current_withdraw,"");
    ...
    isOpen(channel_identifier)
    {
        ...
        channel.settle_block_number += uint256(now);
        ...
        require(closing_participant == recovered_closing_participant_address,"closing_participant check failed! ");
        ...
        // lose the assets that were transferred to him.
        if (nonce > 0) {
            ...
            require(non_closing_participant == recovered_non_closing_participant_address,"non_closing_participant check failed");
        ...
        external
    {
        ...
        require(channel.state == ChannelState.Closed,"channel closed!!!");
        ...
        require(channel.settle_block_number >= now, "settle block_number wrong ");
        ...
        require(non_closing_participant == recovered_non_closing_participant, "non_closing_participant check failed");
        ...
        require(closing_participant == recovered_closing_participant, "closing_participant check failed");
        ...
        require(closing_participant_state.is_the_closer, "not closer");
    ...
    /// @param participant1_transferred_amount The latest known amount of assets
    /// transferred from `participant1` to `participant2`.
    /// @param participant1_locked_amount Amount of assets owed by
    ...
    /// @param participant2_transferred_amount The latest known amount of assets
    /// transferred from `participant2` to `participant1`.
    /// @param participant2_locked_amount Amount of assets owed by
        ...
        public
    {
        ...
        // receive at least the amount of assets that he would have received if
        ...
        require(channel_identifier == getChannelIdentifier(participant1, participant2),"channel_identifier check failed ");
        ...
        require(channel.state == ChannelState.Closed,"channel must be close!");

        // Settlement window must be over
         //       require(channel.settle_block_number < block.number);
       require(channel.settle_block_number < now,"settle_block_number must less than block number");
        ...
        ), "participant1 balance check failed");
        ...
        ), "participant2 balance check failed");

        // We are calculating the final asset amounts that need to be
        // transferred to the participants now and the amount of assets that
        // need to remain locked in the contract. These assets can be unlocked
        // by calling `unlock`.
        // participant1_transferred_amount = the amount of assets that
        //   participant1 will receive in this transaction.
        // participant2_transferred_amount = the amount of assets that
        //   participant2 will receive in this transaction.
        // participant1_locked_amount = the amount of assets remaining in the
        //   contract, representing pending transfers from participant1 to participant2.
        // participant2_locked_amount = the amount of assets remaining in the
...
//        );
        ...
        // Do the actual asset transfers
        if (participant1_transferred_amount > 0) {
            asset.send(participant1, participant1_transferred_amount,"");
        }

        if (participant2_transferred_amount > 0) {
            asset.send(participant2, participant2_transferred_amount,"");
        }
    }

    /// @notice Unlocks all pending off-chain transfers from `sender` to
    /// `receiver` and sends the locked assets corresponding to locks with
    /// secrets registered on-chain to the `receiver`. Locked assets
    ...
    /// the unclaimable unlocked assets.
        ...
        public
    {
        ...
        // assets corresponding to the locked transfers with secrets revealed
        // on chain.
        ...
        // Get the amount of assets that have been left in the contract, to
        ...
        // Make sure we don't transfer more assets than previously reserved in
        // the smart contract.
        unlocked_amount = min(unlocked_amount, locked_amount);

        // Transfer the rest of the assets back to the sender
        returned_assets = locked_amount - unlocked_amount;
        ...
        // Transfer the unlocked assets to the receiver. unlocked_amount can
        // be 0
        if (unlocked_amount > 0) {
            asset.send(receiver, unlocked_amount,"");
        }

        // Transfer the rest of the assets back to the sender
        if (returned_assets > 0) {
            asset.send(sender, returned_assets,"");
        }

        // At this point, this should always be true
        assert(locked_amount >= returned_assets);
    ...
    /// @dev Function that calculates the amount of assets that the participants
        ...
        returns (uint256, uint256, uint256, uint256)
    {
        ...
        // The amount of assets that participant1 MUST receive at the end of
        // the channel lifecycle (after settleChannel and unlock) is:
        // B1 = D1 - W1 + T2 - T1 + Lc2 - Lc1

        // The amount of assets that participant2 MUST receive at the end of
        // the channel lifecycle (after settleChannel and unlock) is:
        ...
        // L1 = total amount of assets locked in pending transfers, sent by
        //   participant1 to participant2
        // L2 = total amount of assets locked in pending transfers, sent by
        ...
        // Notes:
        // 1) The unclaimble assets from a locked amount will return to the sender.
        // At the time of calling settleChannel, the assetNetwork contract does
        ...
        //   assetNetwork contract when calling settleChannel (due to overflows
        ...
        // not send assets from other channels. The only case where TAD is
...
//            message_length,
    ...
    function recoverAddressFromWithdrawMessageForTest(
        uint256 channel_identifier,
        address participant,
        uint256 total_withdraw,
        uint256 expiration_block,
    ...
    public
    view
    returns (address signature_address, bytes32 message_hash)
    {
        // Length of the actual message: 20 + 32 + 32 + 32 + 20 + 32 + 32
        string memory message_length = '200';
    ...
    /// calculates the amount of assets that can be unlocked because the secret

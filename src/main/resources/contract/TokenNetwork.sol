...
contract TokenNetwork is Utils, BAC001Holder{
        ...
        returns (uint256)
    {
        ...
        require(token.balance(address(this)) < token_network_deposit_limit);
        ...
        isOpen(channel_identifier)
    {
        ...
        // Red Eyes release token network limit
        require(token.balance(address(this)) + added_deposit <= token_network_deposit_limit);
        ...
        // Do the transfer
       token.sendFrom(msg.sender, address(this), added_deposit,"");
    }
        ...
        isOpen(channel_identifier)
    {
        ...
        token.send(participant, current_withdraw,"");
...
//        );
        ...
        if (participant1_transferred_amount > 0) {
            token.send(participant1, participant1_transferred_amount,"");
        }

        if (participant2_transferred_amount > 0) {
            token.send(participant2, participant2_transferred_amount,"");
        }
    }
        ...
        public
    {
        ...
        if (unlocked_amount > 0) {
            token.send(receiver, unlocked_amount,"");
        }

        // Transfer the rest of the tokens back to the sender
        if (returned_tokens > 0) {
            token.send(sender, returned_tokens,"");
        }
